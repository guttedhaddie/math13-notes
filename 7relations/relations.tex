...
\begin{examples}{}{}
	...
	\begin{enumerate}\setcounter{enumi}{1}
		...
		\item The relation $\cS=\bigl\{(1,1),(1,3),(3,1),(3,3)\bigr\}$ is not an equivalence relation on $X=\{1,2,3\}$.
		\begin{itemize}
			\item[]\eqrefl False. For instance $(2,2)\not\in\cS$ \ (otherwise said, $2\mathbin{\not\!\cS} 2$).
			\item[]\eqsymm True. In all four cases, $(x,y)\in\cS\Longrightarrow (y,x)\in\cS$ is clear.
...
The proof is an exercise. Observe that a suitable function in the previous example would be
\[
...
\begin{examples}{}{funceqreln}
	...
	\]
	%We could verify this instead using the definition.
% 		\begin{itemize}
% 			\item[]\eqrefl $\forall x\in\Z,\ x\sim x$ since $x^2$ is always congruent to itself!
% 			\item[]\eqsymm $\begin{array}[t]{@{}rl}
% 				\forall x,y\in\Z,\ x\sim y&\implies x^2\equiv y^2\pmod 5\\
% 				&\implies y^2\equiv x^2\pmod 5\\
% 				&\implies y\sim x
% 			\end{array}$
% 			\item[]\eqtrans $\forall x,y,z\in\Z$, if $x\sim y$ and $y\sim z$, then $x^2\equiv y^2$ and $y^2\equiv z^2\pmod 5$. But then $x^2\equiv z^2\pmod 5$ and so $x\sim z$.
% 	\end{itemize}
% 	The equivalence classes for this example are a little tricky. First observe that
% 		\[
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\begin{align*}
				...
				&\implies x,z\in A_n \implies x\sim z \tag*{\qedhere}
			\end{align*}
		\end{itemize}
	\end{enumerate}
\end{proof}
...
\begin{enumerate}
  \item Show that $x\sim y\iff x\equiv y\spmod 3$ defines an equivalence relation on $\Z$. What are the equivalence classes?
  
  
  \item A relation $\cR$ is \emph{antisymmetric} if $\bigl((x,y)\in \cR\bigr)\wedge\bigl((y,x)\in \cR\bigr)\Longrightarrow x=y$. Give examples of relations $\cR$ on $X=\{1,2,3\}$ having the stated property.
	\begin{enumerate}
  ...
  \item For each relation $\cR$ on $\Z$, decide whether $\cR$ is reflexive, symmetric, or transitive, and whether $\cR$ is an equivalence relation.
	\begin{enumerate}
  ...
  \[
  	p\relR q\iff p\text{ and $q$ have a common root}
  \]
  For example $p(x)=(x-1)^2$ and $q(x)=x^2-1$ have the root 1 in common, so $p\relR q$. Determine which of the properties reflexive, symmetric and transitive are possessed by $\cR$.
...
% We call $\cS$ the \emph{reflexive closure} of $\cR$.
	...
	\[
		(a,b)\sim (c,d)\iff \exists\lambda\in\R\setminus\{0\}\ \text{ such that }\ (c,d)=(\lambda a,\lambda b)
	\]
	\begin{enumerate}
	  \item Prove that $\sim$ is an equivalence relation.
	  \item What does this relation have to do with the projective line $\pr(\R^2)$ (page \pageref{ex:projline}).
	\end{enumerate}
...
% \end{enumerate}
  ...
  \item (\emph{If you've studied Linear Algebra})\lstsp Let $\sim$ be the relation on the set $M_2(\R)$ of $2\times 2$ real matrices given by $A\sim B\Longleftrightarrow\exists M$ such that $B=MAM^{-1}$.
  \begin{enumerate}
    \item Prove that $\sim$ is an equivalence relation.
    \item What is the equivalence class of the identity matrix?
    \item Show that $\begin{smatrix}
    	-11&15\\-5&9
    \end{smatrix}
		\sim
		\begin{smatrix}
...
%     \item (Hard) Suppose that $A,B$ have the same, but distinct, eigenvalues $\lambda_1\neq\lambda_2$. Prove that $A\sim B$. \emph{Again use diagonalization, the challenge here is to make your proof work even when the eigenvalues are complex numbers.}
	...
	\end{enumerate}
	%\emph{If Jane and Tom both had the same breakfast and lunch, then $A_{\text{Jane}}=A_{\text{Tom}}$ so there are likely many fewer \emph{distinct} sets $A_x$ than there are humans!}
	...
	\begin{enumerate}
	  ...
	  \item Compute the sets $A_1,A_2,A_3$ where $A_n=\{x\in X:x\relR n\}$. Show that $\{A_1,A_2,A_3\}$ do not form a partition of $X$.
	  \item Repeat parts (a) and (b) for the relations $\cS$ and $\cT$ on $X$, where
	  \begin{gather*}
	  	\cS=\bigl\{(1,1),(1,3),(3,1),(3,3)\bigr\}\\
	  	\cT=\bigl\{(1,1),(1,2),(1,3),(2,1),(2,2),(2,3),(3,3)\bigr\}
	  \end{gather*}
	\end{enumerate}
	(\emph{Warning! Some of the sets $A_1,A_2,A_3$ might be the same in these examples})
	%If, for example, $A_1=A_3$, then the collection $\{A_1,A_2,A_3\}$ only contains two sets: $\{A_1,A_2\}$. Is this a partition? Compare with the example on page \pageref{ex:partnot}.}
	...
	\item\begin{enumerate}
	  \item Prove Theorem \ref{thm:funceqreln}.\footnotemark
  ...
  \item FOr Example not equiv, compute the `class' $\{y:x\sim y\}$. What do you observe?
...
\end{exercises}
	...
	\footnotetext{That the converse also holds is very important in advanced algebra: If $\sim$ is an equivalence relation on $X$, then the \emph{canonical map} $f:X\to\quotient X\sim:x\mapsto [x]$ satisfies $x\sim y\Longleftrightarrow f(x)=f(y)$.}
...
\boldsubsection{Functions and Partitions (Optional) --- include in well-def???}
...
\begin{exercises}{}{}
	%A reading quiz and practice questions can be found \href{http://www.math.uci.edu/~ndonalds/math13/selftest/7-6-funcpart.html}{online}.
