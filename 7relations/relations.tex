...
\begin{examples}{}{notequiv}
	...
	\begin{enumerate}\setcounter{enumi}{1}
		...
		\item\label{ex:notequiv3} The relation $\cS=\bigl\{(1,1),(1,3),(3,1),(3,3)\bigr\}$ is not an equivalence relation on $X=\{1,2,3\}$.
		\begin{itemize}
...
\begin{enumerate}
  \item Show that $x\sim y\Longleftrightarrow x\equiv y\pmod 3$ defines an equivalence relation on $\Z$. What are the equivalence classes?
  ...
  \item For Example \ref*{ex:notequiv}.\ref{ex:notequiv3}, compute the `classes' $[x]=\{y:x\mathbin{\cS}y\}$. What do you observe?
...
% We call $\cS$ the \emph{reflexive closure} of $\cR$.
  ...
  \item\label{exs:anstisymm} A relation $\cR$ is \emph{antisymmetric} if $\bigl((x,y)\in \cR\bigr)\wedge\bigl((y,x)\in \cR\bigr)\Longrightarrow x=y$. Give examples of relations $\cR$ on $X=\{1,2,3\}$ having the stated property.
	...
	OR SYM REF CLOSURE??? AT least abstract????
	
	$\cl{\cR}_{\text{ref}}:=\cR\cup =$, $\cl{\cR}_{\text{sym}}:=\cR\cup\cR^{-1}$
	...
	\begin{enumerate}
	  ...
	  \item Let $A_n=\{x\in X:x\relR n\}$. Show that $\{A_1,A_2,A_3\}$ do not partition $X$.
