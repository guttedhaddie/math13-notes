...
\footnotetext{Be careful!---the colon is the ``such that" separator while $\mid$ denotes the property ``4 divides $x$.''}
...
\footnotetext{This is needed for complements since $x$ has to live somewhere: should, say, $\comp{\{1\}}$ be the set of all \emph{integers} except 1, \emph{real numbers} except 1, etc.?! In many contexts the universal set is naturally assumed: $\cU=\R$ makes sense if you are doing calculus, $\cU=\Z$ if doing modular arithmetic. The universal set is not needed for the rest of the definition (that the union is a set is typically an axiom, while intersections and relative complements are automatically subsets of pre-existing sets).}

In the first Venn diagram, the outer box depicts the universal set $\cU$. Though it doesn't constitute a proof, the second diagram certainly suggests that
\[
