...
In order to read and construct proofs, we need to start with the language in which they are written: \emph{logic.} This is to mathematics what grammar is to English. %The initial parts of this chapter may not appear particularly mathematical, but this will soon change!
...
\begin{example}{}{}
	...
	\end{quote}
	It is typical to modify phrasing to aid readability: ``Not, I like purple'' just sounds weird! Note also that or is \emph{inclusive} in logic: if ``I like purple or chartreuse'' is true, then you might like \emph{both}! 
\end{example}
...
Remember that the expressions $P\Longrightarrow Q$ and $P\Longleftrightarrow Q$ are themselves \emph{propositions}: sentences which are either true or false. Connective propositions can be read and written in many different ways:
\begin{quote}
...
\begin{example}{}{}
	The following sentences all mean exactly the same thing: each expresses, in English, the same conditional $P\implies Q$.
...
\begin{example}{}{}
	...
	\begin{quote}
	  $P$: ``Claudia is holding a peach.''\par
	  $Q$: ``Claudia is holding a piece of fruit.''
	\end{quote}
	The proposition $P\Longrightarrow Q$, namely
	\begin{quote}
		If Claudia is holding a peach, then she is holding a piece of fruit,
	\end{quote}
	is \emph{true,} since a peach is indeed a piece of fruit. The \emph{converse} of $P\Longrightarrow Q$ is the sentence:
	\begin{quote}
	  If Claudia is holding a piece of fruit, then she is holding a peach.
	\end{quote}
	This is palpably false: Claudia could be holding an apple! The \emph{contrapositive} of $P\Longrightarrow Q$ is
	\begin{quote}
	  If Claudia is \emph{not} holding any fruit, then she is \emph{not} holding a peach.
	\end{quote}
...
Our final results in basic logic are attributable to Augustus de Morgan, a famous 19\th{} century logician.
...
\begin{example}{}{}
	...
	This feels awkward in English because the negation encompasses \textcolor{blue}{three distinct possibilities}. Note how the logical (inclusive) use of \emph{or} includes the last row of the truth table: the possibility that you neither rode the subway nor had coffee. This example is indeed an advert for the use of logic: English simply isn't very good for precisely stating complex logical statements. 
\end{example}

\begin{aside}{}{}
\boldinline{Aside: Algebraic Logic}
...
\[
	...
	\end{array}
\]
To make things more algebraic, we've replaced ``is logically equivalent to" with biconditionals.\footnotemark{}\smallbreak
...
\footnotetext{Stating the laws in this fashion is to assert that each expression is a tautology. For instance, to claim that ``$\neg(\neg P)$ is logically equivalent to $P$'' is to assert that $\neg(\neg P)\iff P$ is a tautology.}
