...
The reversed algorithm works in general, yielding a very powerful result. If either of $a,b$ are negative, apply the algorithm to $\nm{a},\nm{b}$ and correct the signs afterwards.
...
\begin{example}{}{2-3div6}
	...
	\]
	While we could have done this example using Theorem \ref{thm:oddodd2}, Bézout proves the \textcolor{blue}{general claim whenever $\gcd(a,b)=1$}: for instance $19\mid c$ and $21\mid c$ together imply $399\mid c$.
\end{example}
...
\begin{thm}{}{diophantine}
	...
	\begin{enumerate}%\itemsep0pt
	  \item The equation $ax+by=c$ has an integer solution $(x,y)$ if and only if $\,d\mid c$. In particular, the values $ax+by$ are precisely the \textbf{integer multiples} of $\gcd(a,b)$.
...
\footnotetext{%
	In future studies, you'll refer to 10 as the \emph{multiplicative inverse of 7 in the ring $\Z_{23}$,} and write $7^{-1}=10$ (see Exercise \ref{exs:ringintro}).%
}
