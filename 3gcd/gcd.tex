...
%   \item A \emph{divisor} $b$ of an integer $a$ is \underline{\phantom{an integer $b$ such that $b\mid a$}\qquad\qquad}
...
\end{quote}
  ...
  %\item What about for the line $4x+6y=1$?
...
In this section we will see how to solve all such \emph{linear Diophantine equations.}\footnote{Equations with integer coefficients and solutions honor the number theorist Diophantus of Alexandria (3\rd{}C.\,\CE).}
%for which lines $ax+by=c$ with $a,b,c\in\Z$, are there integer solutions, and how can we find them all? 
...
%\footnotetext{The astute observer should recognize the similarity between this and the complementary function/particular integral method for linear differential equations: $(x_0,y_0)$ is a `particular solution' to the full equation $ax+by=c$, while $(\frac bdt,-\frac adt)$ comprises all solutions to the `homogeneous equation' $ax+by=0$.}
...
\footnotetext{In future studies, you'll refer to 10 as the \emph{multiplicative inverse of 7 in the ring $\Z_{23}$,} and write $7^{-1}=10$. %This notation isn't advised at the moment, lest you confuse it with $\frac 17$.
}
...
%   \end{enumerate}
  ...
  \item The set of remainders $\Z_n=\{0,1,2,\ldots,n-1\}$ is called a \emph{ring} when equipped with addition and multiplication modulo $n$. For example $5+6\equiv 3\pmod{8}$. We say that $b\in\Z_n$ is an \emph{inverse} of $a\in\Z_n$ if
	...
	\end{enumerate}
		...
		%All integers are divisors of zero, so $\gcd(m,0)=\nm m$. Bézout reads $\gcd(m,0)=m\cdot(\pm 1)+0\cdot y$ for any $y\in\Z$.
...
% 	  
% 	  \begin{enumerate}
% 	    \item $\frac cd,\frac nd$ have no common factors $>1$ since we have divided out by all of them.
% 	    \item $ac\equiv bc\spmod n\implies (a-b)\frac cd=k\frac nd$ for some $k$. By part (a), $k$ is divisible by $\frac cd$, whence $a-b$ is an integer multiple of $\frac nd$.
% 	  \end{enumerate}
\end{enumerate}
