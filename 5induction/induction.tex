...
In this section we think more carefully about the logic behind induction. Why does induction prove what we claim it does? The fact that induction really works depends on a fundamental property of the natural numbers.
...
\end{itemize}
To show that a set $A$ is \emph{not} well-ordered, we need only exhibit a non-empty subset $B$ which has \emph{no minimum.}

\begin{examples}{}{}
	\exstart $A=\{4,-7,\pi,19,\ln 2\}$ is a well-ordered set. There are 31(!) non-empty subsets of $A$, each of which has a minimum element.
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  \[
	  	3<x<b<4 \text{ from which }x\in B\text{ and }x<b
	  \]
...
From the examples, you might be suspect that \emph{well-ordered} simply means \emph{finite}: if is did, it wouldn't be anything like as interesting a concept. There are indeed \emph{infinite} sets which are also well-ordered!

\begin{axiom}{}{}
	$\N$ is well-ordered.\footnotemark
\end{axiom}

\footnotetext{In Peano's axioms, the most commonly encountered axiomatization of the natural numbers, the validity of induction is usually taken to be an axiom in its own right:
	\begin{quote}
...
\boldsubsubsection{Different Base Cases for Induction}
