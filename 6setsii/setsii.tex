...
\begin{exercises}{}{}
	...
	%   \[\bigcup\limits_{f\in C_0(\R)}\negthickspace\negthickspace A_f=[0,1]\qquad\text{and}\qquad\bigcap\limits_{f\in C_0(\R)}\negthickspace\negthickspace A_f=\{0\}.\]
	  ...
	  \begin{enumerate}
		...
		\item\label{exs:modsnowflake} (Hard)\lstsp In this question we construct a fractal shape similar to construction of the von Koch curve from the Cantor set. Starting with the interval $F_0=[0,1]$, repeatedly delete the \emph{third quarter} segment of each interval to obtain a sequence of sets $F_0,F_1,F_2,\ldots$ starting
	  \begin{gather*}  
	  ...
	  \begin{enumerate}
	    ...
	    \item Prove that the intersection $\bigcap_{n=1}^\infty F_n$ does not contain any intervals of positive length.
